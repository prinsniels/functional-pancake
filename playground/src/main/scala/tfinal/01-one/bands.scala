/** We start with the generalization of the Effect
  */
...
object PFunc {
  ...
  final implicit class FunctorOps[F[_]: Functor, A](private val fa: F[A]) {
    @inline def map[B](ab: A => B): F[B] = implicitly[Functor[F]].map(fa)(ab)
  }
}
...
object Algebras {
  ...
  trait BandAlgebra[F[_]] {
    def createOne(name: String): F[Band]

    def readOneById(id: String): F[Option[Band]]

    def updateOne(band: Band): F[Band]

    def deleteOne(id: String): F[Unit]
  }
  ...
  /** Only the interface belongs to the algebra's
    ...
    */
  trait BandRepoAlgebra[F[_]] {
    def addOne(name: String): F[Band]
...
object Interpreters {
  ...
  class BandService[F[_]](
      bandRepo: BandRepoAlgebra[
        F
      ] // this is required to make this implementation work
  ) extends BandAlgebra[F] {

    override def createOne(name: String): F[Band] =
      bandRepo.addOne(name)

    override def readOneById(id: String): F[Option[Band]] =
      bandRepo.readOneById(id)

    override def updateOne(band: Band): F[Band] =
      bandRepo.updateOne(band)

    override def deleteOne(id: String): F[Unit] =
